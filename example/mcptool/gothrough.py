...
class MCPToolDatasetLoader(DatasetLoader):
    """
    适配 MCPToolBenchPP 数据集。
    支持多个 JSON 数组文件，合并为一个条目列表。
    """

    def __init__(self, file_paths: List[str]):
        self.file_paths = file_paths
        self._entries: List[Dict[str, Any]] = []
        self._source_map: Dict[int, str] = {}  # 记录每个条目来自哪个文件
    ...
    def _load_all_files(self):
        for fpath in self.file_paths:
            ...
            if not isinstance(data, list):
                raise ValueError(f"文件 {fpath} 不是 JSON 数组")
            for entry in data:
                idx = len(self._entries)
                self._entries.append(entry)
                self._source_map[idx] = fpath
    ...
    def iter_entries(self) -> Iterator[Tuple[str, Dict[str, Any]]]:
        for idx, entry in enumerate(self._entries):
            yield self._source_map[idx], entry
    ...
    def get_ground_truth_names(self, entry: Dict[str, Any], source_file: str = None) -> List[str]:
        labels = entry.get("function_call_label", [])
...
class Evaluator:
    ...
    def evaluate(self,
        ...
        """
        执行评估，可选打印进度。
        :param total_entries: 条目总数（用于进度计算）
        :param progress_interval: 进度打印间隔（百分比），默认每10%
        """
        detailed = []
        failures = []
        stats = {
...
class ResultWriter:
    ...
    def print_summary_table(results: List[Tuple[str, str, Dict[str, Any]]]):
        print("\n" + "=" * 110)
        header = f"{'Method':<18}{'Alpha':<6}{'Total Rel':>8}{'Total Irr':>8}{'Top1':>8}{'Top3':>8}{'Top5':>8}{'Misjudge':>10}"
        print(header)
        print("=" * 110)
        for method, alpha_str, summary in results:
            print(f"{method:<18}{alpha_str:<6}"
                  ...
                  f"{summary['recall@1']:>8.3f}{summary['recall@3']:>8.3f}{summary['recall@5']:>8.3f}"
...
# ============================================================
# 示例：在 MCPToolBenchPP 上使用（支持多文件）
# ============================================================

if __name__ == "__main__":
    # 文件列表（可包含多个文件）
    DATA_FILES = [
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/file_system/filesystem_0723_single.json",
        # 可按需添加其他文件
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/browser/browser_0724_single_v3.json",
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/finance/finance_0724_single_v3.json",
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/map/map_0717_single_multi_lang_500.json",
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/pay/pay_0723_single.json",
        "/Users/yuanyi/OpenSource/MCPToolBenchPP/data/search/search_0725_single_v2.json"
    ]
    STORE_PATH = "./MCPTool_fsstore"

    loader = MCPToolDatasetLoader(DATA_FILES)
    doc_builder = FunctionDocBuilder()
    ...
    run_benchmark(
        ...
        output_prefix="mcpTool_benchmark",
    )