...
- Task class has a list of sub tasks allows to check other sub tasks.
...
class Task:
    ...
    status, hash chain, subtasks, and LRU status view.
    """
    ...
    def __init__(
        ...
        with tracer.start_as_current_span("Task.__init__") as span:
            ...
            # Record metric
            task_created_counter.add(1, {"status": self._status})

            logger.info(
                "Task created with hash %s, previous hash %s",
                self.hash,
                self._previous_hash,
            )
            logger.debug(
    ...
    def __repr__(self) -> str:
        return f"<Task hash={self.hash[:8]} status={self.status}>"
